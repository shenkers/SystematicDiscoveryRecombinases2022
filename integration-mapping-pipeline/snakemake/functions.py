...
def format_outline(
    ...
    if r1_human_info is not None:
        r1_human_info = (
            r1_human_info["refname"],
            r1_human_info["refstart"],
            r1_human_info["refend"],
            r1_human_info["orient"],
            r1_human_info["clipleft"],
            r1_human_info["clipright"],
            r1_human_info["pident"],
            r1_genome_blacklist,
        )
    else:
        r1_human_info = ["NA"] * 8
    ...
    if r2_human_info is not None:
        r2_human_info = (
            r2_human_info["refname"],
            r2_human_info["refstart"],
            r2_human_info["refend"],
            r2_human_info["orient"],
            r2_human_info["clipleft"],
            r2_human_info["clipright"],
            r2_human_info["pident"],
            r2_genome_blacklist,
        )
    else:
        r2_human_info = ["NA"] * 8
    ...
    if r1_donor_info is not None:
        r1_donor_info = (
            r1_donor_info["refname"],
            r1_donor_info["refstart"],
            r1_donor_info["refend"],
            r1_donor_info["orient"],
            r1_donor_info["clipleft"],
            r1_donor_info["clipright"],
            r1_donor_info["pident"],
        )
    else:
        r1_donor_info = ["NA"] * 7
    ...
    if r2_donor_info is not None:
        r2_donor_info = (
            r2_donor_info["refname"],
            r2_donor_info["refstart"],
            r2_donor_info["refend"],
            r2_donor_info["orient"],
            r2_donor_info["clipleft"],
            r2_donor_info["clipright"],
            r2_donor_info["pident"],
        )
    else:
        r2_donor_info = ["NA"] * 7
    ...
    for read in all_reads:
        ...
        if maps_r1_donor:
            if r1_donor_info["refstart"] < attd_start and r1_donor_info["refend"] > attd_end:
                ...
                attd_start < r1_donor_info["refstart"] < attd_end < r1_donor_info["refend"]
                and r1_donor_info["clipleft"] > 4
            ):
                r1_attd_status = "CLIPPED_LEFT"
            elif (
                r1_donor_info["refend"] > attd_start > r1_donor_info["refstart"] < attd_end
                and r1_donor_info["clipright"] > 4
            ):
        ...
        if maps_r2_donor:
            if r2_donor_info["refstart"] < attd_start and r2_donor_info["refend"] > attd_end:
                ...
                attd_start < r2_donor_info["refstart"] < attd_end < r2_donor_info["refend"]
                and r2_donor_info["clipleft"] > 4
            ):
                r2_attd_status = "CLIPPED_LEFT"
            elif (
                r2_donor_info["refend"] > attd_start > r2_donor_info["refstart"] < attd_end
                and r2_donor_info["clipright"] > 4
            ):
...
def get_percent_ident(seq1, seq2):
    ...
    return (
        matches / len(seq1),
        len(seq1),
        contig_matches / longest_stretch,
        longest_stretch,
        orig_nogaps_seq2.find(seq2),
        orig_nogaps_seq2.find(seq2) + len(seq2),
        orig_nogaps_seq2.find(longest_stretch_seq2),
        orig_nogaps_seq2.find(longest_stretch_seq2) + len(longest_stretch_seq2),
    )
...
def get_genomic_core(s, pos, orient, core_start, core_end, attd_fwd, attd_rev):
    ...
    if orient == "5p":
        ...
        if len(matching_kmers) > 0:
            ...
            align1 = pairwise2.align.localms(
                s_trimmed, attd, match=2, mismatch=-1, open=-2, extend=-2, penalize_end_gaps=False
            )[0]
            (
                pident_trimmed,
                align_length_trimmed,
                ...
                contig_start,
                contig_end,
            ) = get_percent_ident(align1[0], align1[1])
    ...
    align_start_genomic, align_end_genomic, align_cut_dist = get_align_core(
        align1[0], align1[1], pos, orient, core_start, core_end
    )
    ...
    return (
        ...
        contig_start,
        contig_end,
        ";".join(sorted(list(matching_kmers))),
        align1[0],
        align1[1],
    )
...
def kmer_flanks(input, output, params):
    ...
    for c, s, p, o, c5p, c3p in zip(
        find.contig, find.consensus_seq, find.pos, find.orient, find.softclip_count_5p, find.softclip_count_3p
    ):
        name = c + ":" + str(p) + "(" + o + ")"
        read_count = c5p
        if c3p > c5p:
    ...
    tsv_tmp["name"] = (
        tsv_tmp["chrom"]
        + ":"
        + tsv_tmp["align_core_start"].astype(str)
        + "-"
        + tsv_tmp["align_core_end"].astype(str)
        + "("
        + tsv_tmp["orient"]
        + ")"
    )
    ...
    final["locus"] = (
        final["merged_chrom"] + ":" + final["merged_start"].astype(str) + "-" + final["merged_end"].astype(str)
    )
    ...
    outtsv = tsv.merge(
        tsv_tmp[["chrom", "align_core_start", "align_core_end", "locus"]].drop_duplicates(),
...
def get_integration_sites(input, output, params):
    ...
    for locus in loci:
        ...
        name = (
            tsv_filt.chrom
            + ":"
            + str(tsv_filt.align_core_start - params.slop)
            + "-"
            + str(tsv_filt.align_core_end + params.slop)
            + "("
            + tsv_filt.orient
            + ")"
        )
...
def get_site_annotations(input, output, params):
    ...
    shell(
        "cat {} | grep -v locus$ | cut -f6,10,11 | bedtools sort -g {} > {}".format(
            input.kmer_flanks, params.genome_file, bedfile
        )
    )

    gene_overlap = output.annot + ".tmp.gene_overlap.bed"
    shell(
        "bedtools intersect -g {} -sorted -a {} -b {} -wa -wb > {}".format(
            params.genome_file, bedfile, params.genes, gene_overlap
        )
    )
    ...
    shell(
        "bedtools intersect -g {} -sorted -a {} -b {} -wa -wb > {}".format(
            params.genome_file, bedfile, params.exons, exon_overlap
        )
    )
